        ...
        <div class="common-page-card">
                    ...
                    <Table :loading="loading" :columns="columns1" border :data="data1">
                        <template  slot-scope="{ row,index }" slot="color">
                            <template v-if="isEdit">
                                <input style="width:120px" type="color" v-model="formValue[index].value"  />
                            </template>
                            <div v-else :style="`width: 50%; height: 25px; background: ${row.value};`">
                            </div>
                        </template>
            ...
            <Row>
                <Form ref="formInline" inline>
                    <Col span="24">
                        <FormItem prop="user" >
                            <Button v-if="!isEdit" type="warning" icon="ios-build" ghost @click="showEdit">Edit</Button>
                            <Button v-if="isEdit"  type="primary" icon="md-checkmark" @click="saveChanges"> Save </Button>
                        </FormItem>
                    </Col>
                </Form>
            </Row>
