<template>
    <div class="_content">
            ...
            <Row>
                <Col span="24">
                    <Form ref="formInline" inline>
                        <FormItem>
                            <Input type="text" v-model="search" placeholder="Search Route..."  @on-keyup="getSearchCustomer"><Icon type="ios-search" slot="prepend"></Icon></Input>
                        </FormItem>
                        <!-- <FormItem>
                        ...
                        <FormItem>
                            <Button type="primary" @click="$router.push('/add_router')" >Add Router</Button>
                        </FormItem>
                        <!-- <FormItem>
                ...
                <Col :xs="24" :sm="24" :md="24" :lg="24" class="body">
                    <Button type="warning" v-if="childRouteId > 0" size="small" @click="returnBack" ghost> << back</Button>
                    <ul>
                        <li class="li tr">
                            <p>No.</p>
                            <p>Type</p>
                        ...
                        <li class="li active" v-for="(data,index) in data1">
                            <p>{{index + 1}}</p> <p>{{data.type}}</p>  <p>{{data.title}}</p>  <p>
                                <Button  type="warning"  size="small" ghost @click="showDetails(index)">Details</Button>
                                <Button type="warning" v-if="data.type=='submenu'" size="small" ghost @click="showRoute(index)">Routes</Button>
                            </p>
                               <!-- <p></p> -->
                                <p>
            ...
            <div>
                ...
                <div>
                    <Form label-position="top">
                        <FormItem label="Please Enter Your Password" :error="UpdateValue.passwordError"  >
                            <Input type="password" v-model="UpdateValue.password" placeholder="Password"></Input>
                        </FormItem>
                    </Form>
                </div>
        ...
        <Modal v-model="detailsModal" :title="detailsItem.title" :footer-hide="true" :styles="{top: '20px'}">
            <div class="_item_details" >
                <Table border :columns="detailsColum"  :data="detailsItem.data" :show-header="false">
                    <template slot-scope="{row}" slot="value">
                        <a v-if="row.name == 'Customer Facebook'" :href="row.value" target="_blank" rel="noopener noreferrer"><span style="font-size: 18px; margin-right: 10px;"><i class="fab fa-facebook-square"></i></span> <span>{{row.value}}</span></a>
                        <a v-else-if="row.name == 'Customer Instagram'" :href="row.value" target="_blank" rel="noopener noreferrer"><span style="font-size: 18px; margin-right: 10px;"><i class="fab fa-instagram"></i></span><span>{{row.value}}</span></a>
    ...
    export default {
        ...
        methods: {
            ...
            async resetRoutePosition(){
                // return;
                this.data1 = [];
                const response = await this.callApi('put','/app/resetRoutesPosition',this.newData);
            ...
            async returnBack(){
                this.data1 = [];
                this.loading=true
                const response = await this.callApi('get',`/app/parentRoute/${this.childRouteId}`);
                if(response.status == 200){
                    this.data1 = response.data.data
                    response.data.parent_id ? this.childRouteId = response.data.parent_id :  this.childRouteId = 0
                    this.newList();
                }else this.swr();
                this.loading=false
            },
            ...
            async remove(){
                ...
                if(this.UpdateValue.password == '' || this.UpdateValue.password.trim() == ''){
                ...
                const response = await this.callApi('delete',`/app/delete_router/${this.UpdateValue.id}`,ob)
                if(response.status == 200){
            ...
            async getRouters(){
                this.loading=true
                const response = await this.callApi('get',`/app/get_router?id=${this.childRouteId}&search=${this.search}&page=${this.page}&pageSize=${this.pageSize}`)
                if(response.status == 200){
                    this.data1=response.data;
